...
void RouteController::queryShortestPath(int start_stop_id, int end_stop_id) {
    ...
    if (dist[end_stop_id] == numeric_limits<float>::max()) {
        ...
        cout << "Shortest path from " << start_stop_id << " to " << end_stop_id << " with cost " << dist[end_stop_id] << endl;
        vector<int> path;
...
void RouteController::queryCheapestPath(int start_stop_id, int end_stop_id) {
    ...
    if (fare[end_stop_id] == numeric_limits<float>::max()) {
        ...
        cout << "Cheapest path from " << start_stop_id << " to " << end_stop_id << " with fare " << fare[end_stop_id] << endl;
        vector<int> path;
        for (int at = end_stop_id; at != -1; at = prev[at]) {
            path.push_back(at);
        }
